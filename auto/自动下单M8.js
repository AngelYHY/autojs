...
function go(half) {
    ...
    if (half == 0) {
        ...
        log("判断倍数");
        if (zeroOne > 1.5 * oneZero) {
            let threeThree = r.child(12).findOne(id("tv_profit")).text().replace(/%/g, "");
            if (threeThree > 1) {
        ...
        if (oneZero > 1.5 * zeroOne) {
            let threeThree = r.child(3).findOne(id("tv_profit")).text().replace(/%/g, "");
            if (threeThree > 1) {
